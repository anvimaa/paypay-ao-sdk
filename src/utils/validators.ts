...
export class Validators {
  ...
  static validateAmount(amount: unknown, options: AmountValidationOptions = {}): ValidationResult<number> {
    const {
      minAmount = PAYMENT_CONSTANTS.MIN_AMOUNT,
      maxAmount = PAYMENT_CONSTANTS.MAX_AMOUNT,
      currency = 'AOA'
    } = options;
  ...
  static validateTradeNumber(tradeNo: unknown): ValidationResult<string> {
    ...
    if (tradeNo.length < PAYMENT_CONSTANTS.MIN_TRADE_NO_LENGTH || 
      ...
      result.errors.push(
        `Trade number must be between ${PAYMENT_CONSTANTS.MIN_TRADE_NO_LENGTH} and ${PAYMENT_CONSTANTS.MAX_TRADE_NO_LENGTH} characters`
      );
      return result;
    }
