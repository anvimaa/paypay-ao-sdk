...
export class PayPaySDK {
  private readonly config: Required<PayPayConfig>;
  private readonly options: PayPayOptions;
   ...
   */
  async createPayment(request: (PaymentRequest & { paymentMethod: PaymentMethod }) | (MulticaixaPaymentRequest & { paymentMethod: PaymentMethod })): Promise<PaymentResponse> {
    ...
    let payMethod: MulticaixaPayMethod | undefined;
   ...
   */
  async createMulticaixaPayment(request: MulticaixaPaymentRequest): Promise<PaymentResponse> {
    if (!request.phoneNum) {
    ...
    } as PaymentRequest & { paymentMethod: PaymentMethod });
  }

  /**
   * Cria um pagamento por referência
   */
  async createReferencePayment(request: PaymentRequest): Promise<PaymentResponse> {
    return this.createPayment({ 
      ...request, 
   ...
   */
  async createPayPayAppPayment(request: PaymentRequest): Promise<PaymentResponse> {
   ...
   */
  async orderStatus(outTradeNo: string): Promise<OrderStatusResponse> {
    Validators.validate('outTradeNo', outTradeNo, Validators.validateTradeNumber);
    ...
    return this._send('trade_query', bizContent) as Promise<OrderStatusResponse>;
  }

  /**
   * Cancela um pedido de pagamento
   */
  async closeOrder(outTradeNo: string): Promise<PaymentResponse> {
    Validators.validate('outTradeNo', outTradeNo, Validators.validateTradeNumber);
   ...
   */
  private async _send(service: string, bizContentObj: any): Promise<PaymentResponse> {
    const bizContent = CryptoUtils.encryptWithPrivateKey(
    ...
    Object.keys(params)
      ...
      .forEach((key) => {
        const value = params[key as keyof ApiRequestParams];
        if (value !== undefined) {
    ...
    try {
      const response: AxiosResponse = await axios.post(this.config.apiUrl, body, {
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          throw ApiError.fromApiResponse(
            ...
            error.response.data,
            { service, bizContent: bizContentObj }
          );
   ...
   */
  static validateAngolaPhoneNumber = Validators.validateAngolaPhoneNumber;

  /**
   * Valida valor de pagamento
   */
  static validateAmount = Validators.validateAmount;

  /**
   * Valida número de transação
   */
  static validateTradeNumber = Validators.validateTradeNumber;

  /**
   * Valida email
   */
  static validateEmail = Validators.validateEmail;

  /**
   * Valida IP
   */
  static validateIP = Validators.validateIP;

  /**
   * Valida URL
   */
  static validateURL = Validators.validateURL;

  /**
   * Valida assunto
   */
  static validateSubject = Validators.validateSubject;

  /**
   * Obtém IP público
   */
  static getIp = Helpers.getPublicIp;
}