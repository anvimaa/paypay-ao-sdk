...
class PayPaySDK {
    ...
    async createPayPayAccountPayment({ outTradeNo, amount, payeeAccount, subject = "PAYPAY Transfer", payerIp }) {
        // Validate required parameters
        if (!outTradeNo) throw new Error("outTradeNo is required");
        if (!amount) throw new Error("amount is required");
        ...
        const bizContent = {
            ...
            pay_method: {
                pay_product_code: "33", // Different product code for PAYPAY account transfers
                amount: Number(amount).toFixed(2),
