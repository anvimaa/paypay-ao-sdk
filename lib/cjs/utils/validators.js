...
class Validators {
    ...
    static validateAngolaPhoneNumber(phoneNum) {
        ...
        for (const pattern of types_1.PAYMENT_CONSTANTS.ANGOLA_PHONE_PATTERNS) {
    ...
    static validateAmount(amount, options = {}) {
        const { minAmount = types_1.PAYMENT_CONSTANTS.MIN_AMOUNT, maxAmount = types_1.PAYMENT_CONSTANTS.MAX_AMOUNT, currency = 'AOA' } = options;
        const result = {
            isValid: false,
            formatted: undefined,
            errors: []
    ...
    static validateTradeNumber(tradeNo) {
        ...
        if (tradeNo.length < types_1.PAYMENT_CONSTANTS.MIN_TRADE_NO_LENGTH ||
            tradeNo.length > types_1.PAYMENT_CONSTANTS.MAX_TRADE_NO_LENGTH) {
            result.errors.push(`Trade number must be between ${types_1.PAYMENT_CONSTANTS.MIN_TRADE_NO_LENGTH} and ${types_1.PAYMENT_CONSTANTS.MAX_TRADE_NO_LENGTH} characters`);
            return result;
        }
    ...
    static validateIP(ip) {
        ...
        if (!validator_1.default.isIP(ip)) {
    ...
    static validate(field, value, validator) {
        ...
        if (!result.isValid) {
            throw new errors_1.ValidationError(field, value, 'valid format', result.errors.join('; '));
        }
